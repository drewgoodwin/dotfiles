...
hl.config({
	general = {
		...
		col = {
			active_border = { colors = { "rgba(e78a4eee)", "rgba(a9b665ee)" }, angle = 45 },
...
hl.device({
    name        = "elan06fa:00-04f3:32fd-mouse",
    sensitivity = -0.5,
})

hl.device({
    name        = "elan06fa:00-04f3:32fd-touchpad",
    sensitivity = 0.3,
})
...
hl.bind(mainMod .. " + V", function()
    -- Toggle the floating state of the active window
    hl.dispatch(hl.dsp.window.float({ action = "toggle" }))
    -- Center the currently focused window (which is now floating)
...
hl.bind("SHIFT + PRINT", hl.dsp.exec_cmd("hyprshot -freeze --mode=region --raw --clipboard-only | swappy -f - "))

hl.bind(mainMod .. " + SHIFT", hl.dsp.exec_cmd("hyprlock"))
